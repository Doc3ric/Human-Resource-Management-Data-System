...
**Confirmed gap — Module 0.2 (top-header nav):** `resources/views/layouts/dashboard-app.blade.php` (882 lines) still implements a **left sidebar** (`.sidebar`, `.sidebar-logo`, `.sidebar-link`, `.sidebar-footer`, `.sidebar-user` — both light and `body.dark-mode` variants) plus a `.topbar` that today holds only a page title/clock, not primary navigation. The spec's "deprecate the left slide drawer; move all top-level nav into a persistent horizontal top header; reclaim 15–20% width; hamburger+dropdown below 1024px, never a left drawer" requirement is **not implemented**. This is one shared layout file (low file-count risk) included by essentially every authenticated page (high visual-regression risk across the whole app). Flagged for Phase B/C decision.
