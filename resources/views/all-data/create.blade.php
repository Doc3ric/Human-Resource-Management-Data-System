<x-dashboard-app>
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        {{-- Wizard CSS --}}
        <style>
        .wz-step{text-align:center;flex:1;}
        .wz-num{width:34px;height:34px;border-radius:50%;background:#e2e8f0;display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;margin-bottom:4px;transition:all .3s;}
        .wz-step.wz-active .wz-num{background:#2563eb;color:#fff;box-shadow:0 0 0 4px rgba(37,99,235,.15);}
        .wz-step.wz-done .wz-num{background:#22c55e;color:#fff;}
        .wz-line{flex:1;height:2px;background:#e2e8f0;margin-top:17px;transition:background .3s;}
        .wz-line.wz-done{background:#22c55e;}
        .wz-lbl{font-size:11px;font-weight:600;color:#94a3b8;}
        .wz-step.wz-active .wz-lbl{color:#2563eb;}
        .wz-step.wz-done .wz-lbl{color:#16a34a;}
        .wizard-step{display:none;}
        .wizard-step.wz-visible{display:block;}
        </style>

        {{-- Wizard Progress Bar --}}
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-2">
            <div style="display:flex;align-items:center;">
                <div class="wz-step wz-active" id="wz-s1"><div class="wz-num">1</div><div class="wz-lbl">Position Details</div></div>
                <div class="wz-line" id="wz-l1"></div>
                <div class="wz-step" id="wz-s2"><div class="wz-num">2</div><div class="wz-lbl">Employee Info</div></div>
                <div class="wz-line" id="wz-l2"></div>
                <div class="wz-step" id="wz-s3"><div class="wz-num">3</div><div class="wz-lbl">Review &amp; Submit</div></div>
            </div>
        </div>
        ...
        <form id="create-record-form" method="POST" action="{{ route('all-data.store') }}" class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        @endphp
                        <div class="flex flex-col gap-2">
                            <select id="pos_title_select" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 bg-white outline-none" onchange="handlePosTitleChange(this)">
                                <option value="">Select position title...</option>
                                @foreach($knownTitles as $pt)
                                    <option value="{{ $pt }}" {{ $oldPosTitle === $pt ? 'selected' : '' }}>{{ $pt }}</option>
                                @endforeach
                                <option value="Others" {{ $isPosOther ? 'selected' : '' }}>Others (type manually)</option>
                            </select>
                            <input type="text" name="position_title" id="pos_title_input"
                                value="{{ $oldPosTitle }}"
                    ...
                    <div>
                        ...
                        <select name="position_classification" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-white">
                            ...
                            <option value="EXECUTIVE/MANAGERIAL" {{ old('position_classification')==='EXECUTIVE/MANAGERIAL'?'selected':'' }}>Executive/Managerial</option>
                    ...
                    <div>
                        ...
                        <input type="number" name="salary_grade" id="salary_grade" min="1" max="33" value="{{ old('salary_grade') }}"
                    ...
                    <div>
                        ...
                        <select name="step" id="step" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-white">
                            <option value="">Select step...</option>
                            @for($s=1;$s<=8;$s++)
                            <option value="{{ $s }}" {{ (int)old('step',1)===$s?'selected':'' }}>Step {{ $s }}</option>
                            @endfor
                        </select>
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Authorized Annual Salary <span class="text-gray-400 text-[10px] ml-1">(Calculates from SG if blank)</span></label>
                        <input type="number" step="0.01" name="authorized_annual_salary" id="authorized_annual_salary" value="{{ old('authorized_annual_salary') }}"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    </div>

                    <div>
                        <label class="block text-xs font-medium text-gray-600 mb-1">Actual Annual Salary <span class="text-gray-400 text-[10px] ml-1">(Calculates from SG if blank)</span></label>
                        <input type="number" step="0.01" name="actual_annual_salary" id="actual_annual_salary" value="{{ old('actual_annual_salary') }}"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="employment_status" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none bg-white">
                            ...
                            <option value="CT"     {{ old('employment_status')==='CT'    ?'selected':'' }}>Co-Terminous</option>
                            ...
                            <option value="JO"     {{ old('employment_status')==='JO'   ?'selected':'' }}>Job Order</option>
                        </select>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex justify-between items-center mb-1 pb-2 border-b border-gray-100">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer bg-gray-50 px-3 py-1.5 rounded-md border border-gray-200">
                        <input type="checkbox" id="explicit_vacant_toggle" class="rounded w-4 h-4 text-blue-600 focus:ring-blue-500">
                ...
                <div id="employee_fields_container" class="transition-opacity duration-200">
                    <p class="text-xs text-gray-400 mb-4">Leave blank or check the box above if this is a vacant position.</p>
                    ...
                    <div>
                        ...
                        <input type="date" name="date_original_appointment" value="{{ old('date_original_appointment') }}"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    </div>
                    <div>
                ...
                <div class="flex flex-wrap gap-6 mt-4 items-center">
                    ...
                    <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer" title="Eligible for Magna Carta benefits (e.g. Pre-Retirement NOSA)">
                        <input type="checkbox" name="is_health_worker" value="1" {{ old('is_health_worker')?'checked':'' }} class="rounded text-pink-600 focus:ring-pink-500">
                        <span class="text-pink-700 font-semibold"><i class="bi bi-heart-pulse-fill"></i> Health Worker</span>
                    </label>
                    {{-- Vacant status is auto-detected: if no employee name is entered, the position is automatically marked as Vacant --}}
                    <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                        <input type="checkbox" name="abolished" value="1" {{ old('abolished')?'checked':'' }} class="rounded"> Abolished
                    </label>
                    <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                        <input type="checkbox" name="dissolved" value="1" {{ old('dissolved')?'checked':'' }} class="rounded"> Dissolved
                    </label>
                    <div class="flex flex-col gap-2 p-3 bg-orange-50 rounded-lg border border-orange-200">
                        <label class="flex items-center gap-2 text-sm font-semibold text-orange-800 cursor-pointer" title="Excludes employee from NOSI/NOLP step increment processing">
                            <input type="checkbox" name="is_apprehended" id="is_apprehended" value="1" {{ old('is_apprehended')?'checked':'' }} class="rounded text-orange-600 focus:ring-orange-500"> ⚠ Apprehended
                        </label>
                        <div id="apprehended_dates" class="{{ old('is_apprehended') ? '' : 'hidden' }} flex items-center gap-2 pl-6">
                            <span class="text-xs text-orange-700 font-medium">From:</span>
                            <input type="date" name="apprehended_from" value="{{ old('apprehended_from') }}" class="border border-orange-300 rounded-lg px-2 py-1 text-xs focus:ring-2 focus:ring-orange-500 outline-none bg-white">
                        </div>
                    </div>

                    <div class="flex flex-col gap-2 p-3 bg-red-50 rounded-lg border border-red-200">
                        <label class="flex items-center gap-2 text-sm font-semibold text-red-800 cursor-pointer" title="Excludes employee from NOSI/NOLP step increment processing">
                            <input type="checkbox" name="is_admin_charge" id="is_admin_charge" value="1" {{ old('is_admin_charge')?'checked':'' }} class="rounded text-red-600 focus:ring-red-500"> ⚠ Admin Charge
                        </label>
                        <div id="admin_charge_details" class="{{ old('is_admin_charge') ? '' : 'hidden' }} flex flex-col gap-2 pl-6 mt-2">
                            @php
                            ...
                            <div id="admin_charges_list" class="flex flex-col gap-3">
                                ...
                                <div class="admin-charge-row flex flex-wrap items-start gap-2 bg-white p-2 rounded-lg border border-red-100 relative">
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] text-red-700 font-bold uppercase">From:</span>
                                        <input type="date" name="admin_charges[{{ $index }}][from]" value="{{ $charge['from'] ?? '' }}" class="border border-red-300 rounded-lg px-2 py-1 text-xs focus:ring-2 focus:ring-red-500 outline-none w-[110px]">
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] text-red-700 font-bold uppercase">To:</span>
                                        <input type="date" name="admin_charges[{{ $index }}][to]" value="{{ $charge['to'] ?? '' }}" class="border border-red-300 rounded-lg px-2 py-1 text-xs focus:ring-2 focus:ring-red-500 outline-none w-[110px]">
                                    </div>
                                    <div class="flex flex-col gap-1 flex-1 min-w-[200px]">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <select class="admin-charge-type-select border border-red-300 rounded-lg px-2 py-1 text-xs focus:ring-2 focus:ring-red-500 outline-none bg-white flex-1">
                                                <option value="" {{ empty($cType) ? 'selected' : '' }}>Select...</option>
                                                ...
                                                <option value="Others" {{ $isOther ? 'selected' : '' }}>Others (Please Specify)</option>
                                            </select>
                                        </div>
                                        <input type="text" name="admin_charges[{{ $index }}][type]" value="{{ $cType }}" placeholder="Specify type..." class="admin-charge-type-input border border-red-300 rounded-lg px-2 py-1 text-xs focus:ring-2 focus:ring-red-500 outline-none w-full {{ $isOther ? '' : 'hidden' }}">
                                    </div>
                            ...
                            <button type="button" id="btn_add_charge" class="self-start mt-1 text-xs font-semibold text-red-700 bg-white border border-red-300 hover:bg-red-50 px-3 py-1.5 rounded-lg transition shadow-sm">
                                + Add Another Charge
                            </button>
                        </div>
                    </div>
            ...
            <div class="wizard-step" id="wizard-step-3">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h2 class="text-base font-semibold text-gray-800 mb-1 pb-2 border-b border-gray-100">Review &amp; Submit</h2>
                    <p class="text-sm text-gray-500 mb-4">Please review the key details below before creating the record.</p>
                    <div id="wz-review-body" class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4"></div>
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-800">
                        <i class="bi bi-info-circle-fill me-1"></i> Scroll up if you need to edit anything using the <strong>Back</strong> button. When ready, click <strong>Create Record</strong>.
                    </div>
                </div>
            </div>{{-- /wizard-step-3 --}}
            ...
            <div class="flex justify-between items-center pt-2">
                <a href="{{ route('all-data.index') }}" class="px-4 py-2 border border-gray-300 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50 transition">Cancel</a>
                <div class="flex gap-3">
                    <button type="button" id="wz-prev-btn" onclick="wizPrev()" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 transition" style="display:none;">← Back</button>
                    <button type="button" id="wz-next-btn" onclick="wizNext()" class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition">Next →</button>
                    <button type="submit" id="wz-submit-btn" class="px-5 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-sm font-medium transition" style="display:none;">✓ Create Record</button>
                </div>
            </div>
        </form>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (itemCodeSelect) {
                itemCodeSelect.addEventListener('change', function() {
                    ...
                    const row = vacantItemsData.find(r => r.item === selectedItem);
                    if (!row) return;

                    // Auto-fill Salary Grade
                    if (sgInput2 && row.salary_grade) {
                        sgInput2.value = row.salary_grade;
                        sgInput2.dispatchEvent(new Event('change', { bubbles: true })); // trigger salary fetch

                        // Subtle flash to indicate the field was auto-filled
                        sgInput2.style.transition = 'background .4s';
                        sgInput2.style.background = '#e0e7ff';
                        setTimeout(() => sgInput2.style.background = '', 900);
                    }

                    // Auto-fill Position Title
                    if (row.position_title && posTitleSelect && posTitleInput) {
                        // Try to find it in the existing select options
                        let found = false;
                        Array.from(posTitleSelect.options).forEach(opt => {
                            if (opt.value === row.position_title) {
                                posTitleSelect.value = row.position_title;
                                posTitleInput.classList.add('hidden');
                                posTitleInput.value = row.position_title;
                                found = true;
                            }
                        });
                        if (!found) {
                            // Select "Others" and type into the manual input
                            posTitleSelect.value = 'Others';
                            posTitleInput.classList.remove('hidden');
                            posTitleInput.value = row.position_title;
                        }

                        // Flash the field
                        const flashEl = found ? posTitleSelect : posTitleInput;
                        flashEl.style.transition = 'background .4s';
                        flashEl.style.background = '#e0e7ff';
                        setTimeout(() => flashEl.style.background = '', 900);
                    }
                });
            }
