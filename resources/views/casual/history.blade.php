<x-dashboard-app>
    <style>
        .hist-hero { background:linear-gradient(135deg,#831843,#be185d); border-radius:14px; padding:22px 28px; margin-bottom:22px; display:flex; align-items:center; justify-content:space-between; gap:16px; }
        ...
        .hist-table thead tr { background:linear-gradient(135deg,#831843,#be185d); }
        ...
        .hist-table tbody tr:nth-child(even) td { background:#fdf2f8; }
        .hist-table tbody tr:hover td { background:#fce7f3 !important; }
    ...
    <div class="hist-hero">
        <div>
            <h1><i class="bi bi-clock-history me-2"></i>Casual Import History</h1>
            <p>Records of all past Casual data imports</p>
        </div>
        <a href="{{ route('casual.index') }}"
           style="display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.25);color:#fff;padding:8px 16px;border-radius:8px;font-size:12px;font-weight:700;text-decoration:none;">
            <i class="bi bi-arrow-left"></i> Back to Casual
        </a>
    </div>
            ...
            <tbody>
                @forelse($logs as $log)
                    @php
                        ...
                        $isUndo  = $log->action === 'Undid Casual Import';
                    @endphp
                    <tr>
                        ...
                        <td>
                            @if(!$isUndo && $created > 0)
                                <form method="POST"
                                      action="{{ route('casual.import.undo', $log->id) }}"
