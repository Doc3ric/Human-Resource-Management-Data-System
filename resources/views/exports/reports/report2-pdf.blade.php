...
.group-hdr td { background:#334155; color:#fff; font-weight:700; padding:5px 8px; font-size:7.5px; text-transform:uppercase; letter-spacing:.5px; }
</style>
</head>
<body>
...
<table>
    ...
    <tbody>
        ...
        <tr class="group-hdr">
            <td colspan="5">Status: {{ strtoupper($cat) }} ({{ count($items) }} personnel)</td>
        </tr>
        @foreach($items as $idx => $item)
        @php $r = $item['record']; @endphp
        <tr>
            <td style="color:#888;">{{ $idx + 1 }}</td>
            <td style="font-weight:600;text-transform:uppercase;">{{ strtoupper($r->last_name ?? '') }}, {{ $r->first_name ?? '' }}</td>
        ...
        @endforeach
        @empty
        <tr><td colspan="5" style="text-align:center;padding:12px;color:#666;font-style:italic;">No active personnel records found.</td></tr>
        @endforelse
    </tbody>
</table>
