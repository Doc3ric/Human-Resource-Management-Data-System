...
<style>
    ...
    .vf-hero .pos-name { color: #d1d5db; font-size: 14px; font-weight: 600; margin: 4px 0 0; }
    ...
    .vf-stat-value { font-size: 28px; font-weight: 900; color: #374151; line-height: 1; }
    ...
    .vf-table tbody tr:nth-child(even) td { background: #f9fafb; }
    .vf-table tbody tr:hover td { background: #f3f4f6 !important; transition: background .1s; }
    .vf-table td {
    ...
    .item-code { font-family: ui-monospace, monospace; color: #4b5563; font-weight: 600; font-size: 11px; }
    .sg-badge {
    ...
    .vf-empty-icon {
        ...
        background: #f3f4f6; width: 72px; height: 72px; border-radius: 50%;
        display: inline-flex; align-items: center; justify-content: center;
        color: #9ca3af;
    }
...
<div class="vf-hero">
    <div class="vf-hero-inner">
        <div>
            <h1><i class="bi bi-x-circle me-2"></i>Vacant Unfunded — Position Detail</h1>
            <div class="pos-name">{{ $position ?: 'All Positions' }}</div>
            <div class="sub">{{ $records->count() }} abolished/unfunded slot(s)</div>
        </div>
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
            <a href="{{ route('plantilla.vacant-unfunded.detail.export.excel', ['position' => $position]) }}"
               class="export-btn excel">
                <i class="bi bi-file-earmark-excel-fill"></i> Export Excel
            </a>
            <a href="{{ route('plantilla.vacant-unfunded.detail.export.pdf', ['position' => $position]) }}"
...
<div class="vf-summary">
    <div class="vf-stat">
        <div class="vf-stat-label">Unfunded Slots</div>
        <div class="vf-stat-value">{{ number_format($totalSlots) }}</div>
        <div class="vf-stat-sub">Abolished/zero-salary positions</div>
    </div>
    <div class="vf-stat">
...
<div class="vf-table-wrap">
    <div class="vf-table-header">
        <div class="vf-table-header-title">
            ...
            All Unfunded Slots for "{{ $position }}"
        </div>
            ...
            <tbody>
                    ...
                    <tr>
                        <td colspan="9">
                            <div class="vf-empty">
                                ...
                                <div style="font-size:12px;">No vacant unfunded positions match this title.</div>
                            </div>
                        </td>
                    </tr>
                @endforelse
                @if($records->isNotEmpty())
                    <tr class="vf-total-row">
                        ...
                        <td class="salary-cell" style="color:#374151;">₱{{ number_format($totalSalary, 2) }}</td>
                        <td colspan="3"></td>
                    </tr>
                @endif
            </tbody>
