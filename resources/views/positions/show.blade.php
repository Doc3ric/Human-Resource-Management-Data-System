...
<body>
        ...
        <div class="detail-container">
            <div class="detail-group">
                ...
                <div class="detail-value">
                    <span class="code-box">{{ $position->code }}</span>
                </div>
            </div>

            <div class="detail-group">
                <span class="detail-label">Department</span>
                <div class="detail-value">{{ $position->organizationalUnit->name }}</div>
            </div>

            <div class="detail-group">
                <span class="detail-label">Salary Grade</span>
                <div class="detail-value">{{ $position->salary_grade ?? 'Not specified' }}</div>
            </div>
            ...
            <div class="detail-group">
                ...
                <div class="detail-value">
                    @if ($position->abolished)
                        <span class="status-badge badge-abolished">
            ...
            @if ($position->appointments->count() > 0)
                <div style="margin-top: 32px;">
                    <div class="section-title">Current Appointments</div>
                    @foreach ($position->appointments as $appointment)
                        <div class="appointment-item">
                            ...
                            @if ($appointment->appointment_start)
                                <div style="font-size: 12px; color: #6b7280; margin-top: 4px;">
                                    <i class="bi bi-calendar"></i> 
                                    {{ $appointment->appointment_start->format('M d, Y') }}
                                </div>
                            @endif
                        </div>
                    @endforeach
            ...
            @else
                <div style="margin-top: 32px;">
                    ...
                    <div class="no-appointments">
                        ...
                        No appointments for this position
                    </div>
                </div>
            @endif

            <div class="meta-info">
                <p><strong>Created:</strong> {{ $position->created_at->format('F d, Y \a\t h:i A') }}</p>
                <p><strong>Last Updated:</strong> {{ $position->updated_at->format('F d, Y \a\t h:i A') }}</p>
            </div>
    ...
    <div class="main-content">
        ...
        <div class="detail-container">
            ...
            <div class="actions">
                ...
                <a href="{{ route('positions.index') }}" class="btn-action btn-back">
                    <i class="bi bi-arrow-left"></i>
                    Back to List
                </a>
                @if (auth()->user()->isSuperAdmin() || auth()->user()->isInventoryAdmin())
                <form action="{{ route('positions.destroy', $position->id) }}" method="POST" id="form-delete-position-show" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="button" onclick="confirmDeletePositionShow()" class="btn-primary-custom" style="background-color: transparent; border: 1px solid #fee2e2; color: #dc2626;">
    ...
    <script>
        function confirmDeletePositionShow() {
            Swal.fire({
                title: 'Delete Position?',
                text: "Are you sure you want to delete this position? This cannot be undone.",
                icon: 'warning',
                iconColor: '#dc3545',
                ...
                if (result.isConfirmed) {
                    document.getElementById('form-delete-position-show').submit();
                }
            });
        }
    </script>
