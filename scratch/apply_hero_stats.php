...
// Fallback CSS if regex fails to capture everything from casual
...
foreach ($filesToProcess as $key => $config) {
    ...
    if ($key === 'job-orders') {
        // Find <div class="jo-stats-grid">...</div>
        $content = preg_replace('/<div class="jo-stats-grid">.*?<\/div>\s*<\/div>\s*<\/div>\s*@endforeach\s*<\/div>/s', $newHtml, $content);
        
        // Also remove Flash messages from below if they were duplicated
        $content = preg_replace('/\{\{-- Flash messages --\}\}\s*@if\(session\(\'success\'\)\).*?@endif/s', '', $content, 1);
        
        // Clean up empty <div class="jo-page"> at top if it was replaced poorly
        if (strpos($content, '<div class="jo-page">') !== false) {
           $content = str_replace('<div class="jo-page">', '', $content);
           $content = preg_replace('/<\/div>\s*$/s', '', $content); // remove an ending div
        }
    } elseif ($key === 'all-data') {
        ...
        // {{-- Hero --}}
        // <div class="perm-hero"> ... </div>
        // {{-- Flash messages --}} ... 
        // {{-- Stats bar --}}
        // <div class="perm-stats"> ... </div>
        
        $content = preg_replace('/\{\{-- Hero --\}\}\s*<div class="perm-hero">.*?\{\{-- Stats bar --\}\}\s*<div class="perm-stats">.*?<\/div>\s*<\/div>\s*<\/div>/s', $newHtml, $content);
        $content = preg_replace('/\{\{-- Hero --\}\}\s*<div class="perm-hero">.*?<\/div>\s*<\/div>\s*\{\{-- Stats bar --\}\}\s*<div class="perm-stats">.*?<\/div>\s*<\/div>\s*<\/div>/s', $newHtml, $content);
    }
